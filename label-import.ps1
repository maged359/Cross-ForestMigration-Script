...
====================================================================
 import.ps1 : LABELS + ENCRYPTION + CONTENT MARKING + STATIC &
              DYNAMIC WATERMARK + POLICIES   (TENANT B)
====================================================================
 Applies EVERYTHING from export.ps1:
   ...
   3. Apply DYNAMIC WATERMARKING (requires encryption + admin-defined
      permissions; applied AFTER encryption is set).        <-- NEW
   4. Create label policies (scoping remapped).
 ...
 Requirements:
   - Tenant B licensed for sensitivity labels (E5/E5 Compliance or trial).
   - Users/groups referenced by rights exist in Tenant B.
   - Dynamic watermarking only applies to labels with admin-defined
     encryption permissions (Microsoft platform rule).

 Usage:
   pwsh
   .\import.ps1 -WhatIf
   .\import.ps1
   .\import.ps1 -SkipEncryption   # everything except encryption + dynamic WM
====================================================================
#>
...
foreach ($l in $ordered) {
    ...
    if ($existingByDisplay.ContainsKey($l.DisplayName) -or $existingByName.ContainsKey($internalName)) {
        ...
        if ($l.HeaderEnabled){
            ...
            if($l.HeaderText){$params.ApplyContentMarkingHeaderText=$l.HeaderText}
            if($l.HeaderFontName){$params.ApplyContentMarkingHeaderFontName=$l.HeaderFontName}
            if($l.HeaderFontSize){$params.ApplyContentMarkingHeaderFontSize=[int]$l.HeaderFontSize}
            if($l.HeaderFontColor){$params.ApplyContentMarkingHeaderFontColor=$l.HeaderFontColor}
            if($l.HeaderAlignment){$params.ApplyContentMarkingHeaderAlignment=$l.HeaderAlignment}
            if($l.HeaderMargin){$params.ApplyContentMarkingHeaderMargin=[int]$l.HeaderMargin}
        }
        if ($l.FooterEnabled){
            $params.ApplyContentMarkingFooterEnabled=$true
            if($l.FooterText){$params.ApplyContentMarkingFooterText=$l.FooterText}
            if($l.FooterFontName){$params.ApplyContentMarkingFooterFontName=$l.FooterFontName}
            if($l.FooterFontSize){$params.ApplyContentMarkingFooterFontSize=[int]$l.FooterFontSize}
            if($l.FooterFontColor){$params.ApplyContentMarkingFooterFontColor=$l.FooterFontColor}
            if($l.FooterAlignment){$params.ApplyContentMarkingFooterAlignment=$l.FooterAlignment}
            if($l.FooterMargin){$params.ApplyContentMarkingFooterMargin=[int]$l.FooterMargin}
        }
        if ($l.WatermarkEnabled){
            $params.ApplyWaterMarkingEnabled=$true
            if($l.WatermarkText){$params.ApplyWaterMarkingText=$l.WatermarkText}
            if($l.WatermarkFontName){$params.ApplyWaterMarkingFontName=$l.WatermarkFontName}
            if($l.WatermarkFontSize){$params.ApplyWaterMarkingFontSize=[int]$l.WatermarkFontSize}
            if($l.WatermarkFontColor){$params.ApplyWaterMarkingFontColor=$l.WatermarkFontColor}
        ...
        if ($WhatIf){
            Write-Host ("WHATIF create: {0} (H={1} F={2} W={3} DynWM={4})" -f $l.DisplayName,$l.HeaderEnabled,$l.FooterEnabled,$l.WatermarkEnabled,$l.DynamicWatermarkEnabled) -ForegroundColor Gray
    ...
    if (-not $SkipEncryption -and $l.EncryptionEnabled) {
        ...
        if ($WhatIf){ Write-Host ("   WHATIF encrypt: rights=[{0}]" -f $rightsString) -ForegroundColor DarkGray; $encryptionApplied=$true }
            ...
            try { Set-Label @encParams -ErrorAction Stop
                  Write-Host ("   ENCRYPTED: rights=[{0}]" -f $rightsString) -ForegroundColor Green
    ...
    # ---------- APPLY DYNAMIC WATERMARK (requires encryption/admin-defined) ----------
...
Write-Host "`nVERIFY in Purview: encryption rights, content marking, static + dynamic watermark, policy scoping." -ForegroundColor Yellow
